...
void debug_ig(t_img *frame, t_gradient *grad)
{
	int x;
	int y;
	...
	while (x < WIDTH)
	{
		...
		while (y < HEIGHT)
		{
			...
			img_set_pxl_color(frame, x, y, get_color_int(get_gradient_color(grad, invlerp(0, WIDTH, x))));
			y++;
		}
		x++;
	}
